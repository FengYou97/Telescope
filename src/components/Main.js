...
const Main = () => {
  ...
  useEffect(() => {
    if ("geolocation" in navigator) {
      ...
      navigator.geolocation.getCurrentPosition(function (position) {
        setCurLat(Number((position.coords.latitude).toFixed(6)));
      ...
      axios.get('http://ipinfo.io/json', null)
        .then(res => {
          if ('loc' in res) {
            ...
            setCurLat(Number((loc[0]).toFixed(2)));
  ...
  const findSatellitesOnClick = (nextObserverInfo) => {
    ...
    setLoading(true);
    fetch(
      `${ABOVE_API_BASE_URL}/${latitude}/${longitude}/${altitude}/${radius}/${SAT_CATEGORY}&apiKey=${NY20_API_KEY}`
    )
